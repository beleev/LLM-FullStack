...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '2 次前向后, VAR 已经有了整张图的大致明暗布局 (先构图再细化)。光栅自回归只画了左上角 2 个像素, 要到第 64 步才知道右下角长什么样。而且光栅的一维展开破坏了二维邻接关系。VAR 总共 4 次前向 (每级内部并行), 光栅要 64 次。代价: VAR 的 token 总数是 1+4+16+64 = 85 > 64, 序列更长。VAR 的每一级又只量化 “残差” (目标减去之前所有级上采样之和), 所以粗尺度的错误可以被细尺度修正。训练时用块状因果 mask: 同一尺度内互相可见, 只能看更粗的尺度。',
    }"
  >
    <template #controls>
