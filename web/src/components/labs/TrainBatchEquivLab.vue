...
<template>
  <LabFrame
    ...
    sub="24 个样本 = 24 个小格。拖卡数和 micro-batch 数换切法, 再按播放。格子一批批点亮, 每张卡右边的梯度缓冲区跟着涨。最后一次 all-reduce 让所有卡握住同一个数。右边四个数字是你为这种切法付的账。"
    ...
    :challenge="{
      ...
      answer: '加卡: 激活峰值和步时间都除以卡数, 与基线的差一直贴在 1e-17 (浮点求和顺序)。只有通信从 0 涨到 2(P−1)/P×梯度字节 —— DDP 的全部买卖就是拿通信换时间。加 micro-batch: 激活峰值除以 K, 步时间一点没省, 通信也没变 —— 累积换的是显存, 不是速度。打开不等分: |Δ| 从 1e-17 跳到 1e-1。all-reduce(mean) 做的是「各卡局部均值的简单平均」, 只有每卡样本数相同时它才等于全局均值。步时间也顺带被最大的那张卡拖长了。真实框架要么用 DistributedSampler 把尾巴补齐或丢掉, 要么把每卡 loss 先乘 n_r·P/N 再同步。',
    }"
  >
    <template #controls>
      <div class="row">
