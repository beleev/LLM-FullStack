...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: 'LTI 下 token j 对最终状态的贡献是 Δ·exp(−Δ·a·(T−1−j)): 越早的 token 被衰减的次数越多。“7” 后面还有 7 个 token, 所以它的占比永远低于它后面的任何一个废话。极限是 Δ→0 时人人平等的 1/10。线性时不变系统只能按 “距离现在多远” 加权, 不能按 “内容重不重要” 加权。Mamba 让 Δ_t = softplus(Linear(x_t)) 依赖输入: 给 “7” 一个大 Δ (写入强、同时 exp(Δ·A)≈0 把之前的状态清掉), 给废话 Δ≈0 (写入≈0 且 exp(Δ·A)≈1 状态原样通过)。一个标量同时控制写入门和遗忘门, 这就是 “选择性”。代价: 系统变成时变的, 不能再用卷积并行, 只能用并行 scan。',
    }"
  >
    <template #controls>
      <div class="row">
