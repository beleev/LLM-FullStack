...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: 'p1=0.5 时树 ≈1.60、链 K=15 ≈1.00: 链第一个猜错后面 14 个全废, 树在最容易错的第一层留了 3 个备胎 (命中率 0.5 → 0.78)。p1=0.9 时反过来, 链 ≈7.1 远超树的 2.90 (树最深只有 3 层) —— draft 足够准时深度比宽度值钱, 所以 EAGLE-2 按置信度动态长树。兄弟节点是同一个位置的不同候选, 最终只有一个会留下, 所以位置都是 n_ctx + depth。mask 保证兄弟之间互相看不见: 每个节点看到的恰好是从根到自己的那条链, 等价于每条路径各跑一次顺序 decode。',
    }"
  >
    <template #controls>
