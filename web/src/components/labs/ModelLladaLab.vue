...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '随机重遮会把没把握的预测也定稿, 而定稿的 token 置信度记为 +∞、永不重遮, 错了就一直错, 还会误导邻居。低置信度重遮让模型先定 “显然” 的位置, 这些位置又成为别人的上下文。难的位置于是留到信息最充分时再定。生成顺序由置信度决定, 而不是从左到右。步数是质量/速度旋钮: steps = 12 时每步只定 1 个, 最稳但要 12 次整段前向。双向注意力没有 KV cache, 每次都重算全部 12 个位置, 总计算是自回归的 12 倍。steps = 1 一次定完, 每个位置都在零上下文下瞎猜。LLaDA 赚的是串行步数可以少于 token 数, 赔的是每步更贵。',
    }"
  >
    <template #controls>
      <div class="row">
