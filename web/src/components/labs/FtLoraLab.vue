...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: 'LoRA 能用 r = 8 替代 d = 4096 的全量更新, 不是因为低秩分解有魔法, 而是因为「微调带来的权重变化本身内在维度就很低」(Aghajanyan 2020 / LoRA 论文的经验发现)。谱衰减快时, 前几个奇异值已经占了几乎全部能量, r = 2 误差就只剩几个百分点; 谱很平时, 误差 = sqrt(被丢掉的 s² 之和 / 全部 s² 之和), 丢多少亏多少。任务离预训练分布越远 (例如学一门新语言), ΔW 越不低秩, 这时就该加大 r 或改用全参。',
    }"
  >
    <template #controls>
      <div class="row">
