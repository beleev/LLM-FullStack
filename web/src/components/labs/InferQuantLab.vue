...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '不是单调的, 是 U 形。X W = (X/s)·(s⊙W): 把离群通道那一行权重放大 s 倍再量化, 它的相对舍入误差缩小约 s 倍; 但它同时撑大了所在 group 的 min/max 范围, 同组其它行的格点变粗。α 太大时「陪绑」的损失超过收益, 所以 α 要在校准集上网格搜索 (真实 demo: INT4 g32 RTN 0.0759 → AWQ 0.0273, 最优 α=0.4)。没有离群通道时各通道 s≈1, 曲线几乎是平的, AWQ 无事可做。group 越小, 离群行只污染更少的邻居, RTN 本身就变好, AWQ 的相对收益缩小 —— 代价是 scale/zero 的开销: 4 + 32/g bit/权重。',
    }"
  >
    <template #controls>
