...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: 'GRPO 的 loss 是 (1/G)·Σ_i (1/|o_i|)·Σ_t Â_i: 错误回答拖到 400 token 后每 token 的惩罚只剩 60/400 = 15%。模型很快发现「反正要错, 不如写长点」, 这就是 R1 类训练里回复越来越长的一部分原因 (不全是「学会了思考」)。Dr.GRPO 把 1/|o_i| 换成常数 1/L_max, 长短一视同仁。Dr.GRPO 同时去掉 /σ: 0/1 奖励下 σ 在「几乎全对 / 几乎全错」的题上最小, 除以 σ 等于给太易太难的题加权。全对时 r − μ 全为 0: 三种算法梯度都是零, 这组采样的算力白花。GRPO 照样把它算进 batch (稀释有效梯度), DAPO 的动态采样则丢掉它继续采, 直到 batch 里每组都有对有错。',
    }"
  >
    <template #controls>
      <div class="row">
