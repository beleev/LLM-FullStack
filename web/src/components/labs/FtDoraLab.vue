...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: 'LoRA 要「只转不伸」, ΔV 必须精确落在那一个点上 —— 弦的另一端。拖偏一点, 长度就跟着变: ΔM 和 ΔD 被同一个旋钮绑死。这正是 DoRA 论文观察到的现象: LoRA 训练中幅度变化与方向变化强正相关。而全参微调里, 两者几乎独立, 甚至负相关。DoRA 里 W′ = m·V/‖V‖。紫点沿射线移动只改变 ‖V‖, 归一化把它消掉了, 所以绿色箭头不动。这说明 loss 对 V 的梯度天然没有径向分量: 它被投影到与 V 垂直的方向, 再乘 m/‖V‖。于是低秩容量全部花在转方向上, 长度交给每个向量一个标量 m。代价是多 d 个参数 —— 相对 LoRA 的 2dr 只多了 1/(2r)。推理前同样可以 merge 回一个普通矩阵。',
    }"
  >
    <template #controls>
      <div class="row">
