...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '要关掉三层。三层是路径围栏、权限门的 auto 分类器 (敏感路径 / 危险命令词)、污点规则。三者互不依赖, 任何一层还在, 私钥都出不去。这就是纵深防御: 每层都假设别的层会失守。注意它们的性质不同。untrusted_data 标记和注入特征检测是在「劝」模型, 换个说法就绕过去了, 属于概率性防御。污点规则、路径围栏、deny 规则不看模型怎么想, 属于确定性防御。一个系统里至少要有一层是后者。全部关掉之后: rm -rf / 真的执行, 私钥外泄, 密钥原样落进 JSONL, 笔记还记错了文档 —— 而模型、工具和 prompt 从头到尾没变过。',
    }"
  >
    <template #controls>
      <div class="row">
