...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '轴对齐时, 每个输入坐标的梯度量级各不相同, Adam 的 1/√v 正好逐坐标把它们拉平 —— 这是 Adam 的主场, 它赢 (换几组数据看, 从小胜到大胜都有)。旋转之后, 病态方向变成了所有坐标的线性组合, 每个坐标上的梯度都混着大小尺度, 逐元素的缩放无从下手; 而 Newton–Schulz 作用在奇异值上, 旋转只改变奇异向量、不改变奇异值, 所以 Muon 的轨迹几乎不变。真实网络的权重没有理由与坐标轴对齐, 这就是 Muon 在 LLM 预训练上省算力的来源。(Python demo 用 32×32: 旋转时 Muon 好 9.2×, 轴对齐时 Adam 好 1.9×。)',
    }"
  >
    <template #controls>
      <div class="row">
