...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '没有。连续切分时卡 3 拿的是序列最后一段, 每一轮收到的 KV 块全在它的过去, 每轮都要算满 64 对; 卡 0 除了第 0 轮以外全部跳过。每轮的耗时 = 最忙的卡, 所以墙钟 = 36 + 64×3 = 228, 相比不用因果性的 256 只省了 11%。zigzag 把序列切成 2D 段, 卡 r 拿第 r 段和第 2D−1−r 段 (一头一尾), 于是每张卡每一轮都恰好是 32 对 (第 0 轮 36), 墙钟 132, 快 1.73 倍。总计算量、通信量完全没变 —— 变的只是负载均衡。',
    }"
  >
    <template #controls>
      <div class="row">
