...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '不补偿时软标签梯度范数大约按 1/T² 掉: T = 8 时只剩 T = 1 时的 2% 左右。于是「α = 0.5」名义上五五开, 实际上几乎全是硬标签 CE 在训练。你以为在蒸馏, 其实在做普通 SFT。乘上 T² 后, 软标签梯度随 T 增大趋于一个常数 (虚线)。这个常数等于两边「去均值 logits 之差」除以 V。高温极限下蒸馏退化成 logits 回归 —— Hinton 2015 的原话。这样 T 只负责「暗知识展开多少」, α 只负责「软硬各占多少」, 两个超参才互不干扰。',
    }"
  >
    <template #controls>
      <div class="row">
