...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: 'KV 依赖它前面的全部 token: 中间节点的 KV 是所有后代前缀的一部分, 扔了它, 后代的 KV 虽然还在显存里却再也匹配不上 (从根走不到), 等于一起作废。所以驱逐顺序只能是叶子 → 变成叶子的父节点 → …。另外注意右侧对比: radix 能命中任意长度 n, 而 block hash 只能命中 ⌊n/bs⌋·bs —— 共享 100 个 token、bs=16 时只能复用 96 个。',
    }"
  >
    <template #controls>
      <div class="row">
