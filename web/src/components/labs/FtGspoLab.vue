...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '每个 ρ_t 都只基于「这个位置采到的这一个 token」, 是方差很大的单样本重要性权重: 噪声 0.3 时常有三四个 token 越界被 clip, 剩下的照常更新 —— 同一条序列、同一个 Â, 有的 token 学有的不学, 而且哪些被 clip 基本是随机的。MoE 模型里路由一变, token 级比率抖得更厉害, 这是 GSPO 论文报告的 GRPO 训练崩溃来源。序列级比率取平均后噪声按 1/√T 缩小, s 通常离 1 很近, 所以 GSPO 的 ε 取得极小 (论文里是 3e-4 ~ 4e-4 量级), 越界时整条序列一起放弃。不做长度归一的 Πρ_t = exp(Σ log ρ_t) 会随长度指数发散或归零, 长序列几乎必然被 clip —— 所以必须取几何平均。',
    }"
  >
    <template #controls>
      <div class="row">
