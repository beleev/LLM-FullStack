...
<template>
  <LabFrame
    ...
    :challenge="{
      ...
      answer: '区域奖励 (token ≥ 5 就给分) 下, 绿框是几条竖带: 每一行可行的输出都一样。所以永远输出 7 的常数策略就是满分 100%。训练曲线很漂亮, 但策略根本没读 prompt: 它学到的是一个 unigram 偏好, 不是任务。看题奖励下每行只有一个绿框, 位置随 prompt 变: 常数策略最多蒙对 4 题里的 1 题 (25%), 要拿满分必须让输出依赖输入。这就是教学用 RL 任务最容易踩的坑: 验证「RL 管线能让 reward 上升」和验证「RL 学会了条件行为」是两回事。真实 RLVR (数学答案、单元测试) 天然是看题的; 自己造玩具任务时要主动保证这一点, 并且单独报告「常数基线」的得分。',
    }"
  >
    <template #controls>
      <div class="row">
