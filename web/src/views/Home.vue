<template>
  <div>
    ...
    <p class="page-subtitle">
      把"训出一个能用、能部署、能行动的大模型系统"拆成 <strong>六个递进阶段</strong>。每个阶段对应仓库根目录下的一个子项目。
      教程与代码逐行对照, 你看到的每一段叙述都能在 Python 文件里找到出处。
    </p>

    <ChapterIntro
      tldr="一条主线, 把模型从公式带到能行动的系统: numpy 手写最小闭环 → 现代架构家族 → 规模化训练 → 任务微调 → 推理优化 → Agent 应用层。"
    ...
    <section class="section">
      ...
      <p class="lead">
        前两阶段看清模型本身。
        中间三阶段解决"怎么训、怎么改、怎么上线"。
        最后一阶段把推理服务接成 Agent —— 能调用工具、保留状态、隔离子任务。
        每张卡都直接连到对应目录的原始代码。
      </p>
