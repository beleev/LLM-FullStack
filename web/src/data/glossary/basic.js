...
export default [
  ...
  t('[B,T,T]', 'attention 分数矩阵', '全模型唯一让不同位置互相说话的张量, 按 T² 长', 'T=64 占激活 14%, T=256 占 40%', 'basic-forward'),
  ...
  t('AdamW', '解耦权重衰减', 'λW 加在更新量上而不是加进梯度, 不被 1/√v 缩放', '1 维 gain/bias 不衰减', 'basic-optim-sample'),
  t('全局范数裁剪', 'grad clip', '所有参数拼成一个长向量同乘一个系数, 梯度方向不变', 'g ← g·min(1, max_norm/‖g‖)', 'basic-optim-sample'),
